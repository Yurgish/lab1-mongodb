...
class SellerController:
    ...
    @handle_errors
    def load(self, store_id: str | None = None, department_id: str | None = None) -> None:
        if store_id is not None and department_id is not None:
            sellers = self._service.get_by_department(store_id, department_id)
        else:
            sellers = self._service.get_all()
